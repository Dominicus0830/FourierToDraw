package org.domi.frame;


import org.domi.fourier.Fourier;
import org.domi.utils.Cycle;
...
public class Frames extends JPanel implements ActionListener {

    JFrame frame;

    public Frames() {
        frame = new JFrame("Domi");
    ...
    public void paint(Graphics g) {
        ...
        g2D.drawString("Bug Test #116", 680, 180);

        g2D.fillRect(200, 200, 700, 700);
        g2D.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.5f));
        g2D.fillRect(1020, 200, 700, 700);
    ...
    public void updateCycleValue() {
        ...
        for (int i = 0; i < fourierX.size(); i++) {
            ...
            if (cyclesX.isEmpty()) {
                cyclesX.add(new Cycle(0, 40, data[0], data[1], data[2], 0));
            } else {
    ...
    private class mouseListener extends JComponent implements MouseMotionListener {
        ...
        public void mouseDragged(MouseEvent e) {
            int x = e.getX();
            int y = e.getY();
            if (x >= 1020 + 3 && x <= 1720 + 3 && y >= 222 && y <= 928) {
                if (update == true) {
                    drawingX.clear();
                    drawingY.clear();
                    update = false;
